...
### Fixed
- **"Prompt is too long" session deadlock** (#93): Claude SDK sessions now run with a lowered auto-compaction threshold (85% of the context window instead of the CLI's 92% default, overridable via `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE`), leaving enough headroom that a max-size HAR read landing near the threshold no longer jumps straight past the hard context limit. When the window is exhausted anyway, the error is now explained (progress is saved on disk; start a new run for the same target to continue) and the follow-up prompt is no longer offered on the dead session, where every further message would fail with the same error.
