...
- Use `System.Net.Http.HttpClient` and `System.Text.Json` — both are part of the .NET base class library (.NET 5+), so no external NuGet package is needed for HTTP or JSON
- Create a minimal project file (`.csproj`, targeting `net8.0`) so the program can be run with a single command
