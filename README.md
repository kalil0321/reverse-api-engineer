...
- **OpenCode setup**: with `sdk: "opencode"`, RAE reuses an existing server or starts `opencode-ai@latest` through `npx`. Before creating a session, it validates the provider/model pair against the server's connected catalog, checks tool support, and suggests currently available free models when configuration is invalid. Compatible older servers are reused with a version warning. Node.js 20+ is required only for automatic startup. Password-protected servers use `OPENCODE_SERVER_PASSWORD` and optional `OPENCODE_SERVER_USERNAME`. Override startup with `OPENCODE_BASE_URL`, `RAE_OPENCODE_PACKAGE`, or `RAE_OPENCODE_AUTO_START=0`.
