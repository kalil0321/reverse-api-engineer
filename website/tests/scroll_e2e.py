...
Run: uv run --extra manual python website/tests/scroll_e2e.py URL EXPORT_DIR [ARTIFACT_DIR]
...
def capture_failure(context, engine, width):
    try:
        ...
        if context.pages:
            try:
                context.pages[-1].screenshot(path=artifacts_dir / f"rae-header-{engine}-{width}-failure.png")
...
parser.add_argument("artifacts_dir", nargs="?", type=Path, default=Path(tempfile.gettempdir()))
args = parser.parse_args()
base_url, export_dir, artifacts_dir = args.base_url, args.export_dir, args.artifacts_dir
            ...
            for width, height in [(1280, 800), (390, 844)]:
                with closing(browser.new_context(viewport={"width": width, "height": height})) as context, capture_failure(context, engine, width):
                    ...
                    for route in routes:
                        ...
                        content_bounds = page.locator("main").first.bounding_box()
                        assert content_bounds["y"] >= header_bounds["y"] + header_bounds["height"] - 1, (
                            engine,
                            route,
                        ...
                        for delta in [3000, -1800, 8000, -9000]:
                            ...
                            if can_move:
                                assert any(abs(p["y"] - before) > 1 for p in positions), (
                                    engine,
                                    route,
                                    delta,
                            ...
                            assert all(abs(p["top"]) < 1 and p["bottom"] > 40 and p["left"] >= -1 and p["right"] <= width + 1 for p in positions), (
                                engine,
                                route,
                                positions,
                            )
                    ...
                    page.screenshot(path=artifacts_dir / f"rae-header-{engine}-{width}.png")
